...
```
❌ 错误（重复）：
  3.9 A/C Wind 表格中写"风向范围 1°~360°、飞行冻结渐变"
  3.16 飞机风键盘 表格中又写一遍"风向范围 1°~360°、飞行冻结渐变"

✅ 正确（分层）：
  3.9 A/C Wind 表格只写：
    - 功能定义：主界面字段，显示当前飞机位置的风向和风速
    - 交互方式：点击字段 → 弹出飞机风键盘（详见 3.16）
...
| 2026-06-02 | V1.5 | 重构第 3 章 HTML 页面结构模板：由「九大章节」改为「静态认知层 → 动态推导层 → 验证层」三层架构；明确功能层级图必须标注触发关系；1.2 核心操作流程图后移至第三章；复杂面板（SLEW/ICING/TURBULENCE）必须内嵌子流程图/状态机 |
