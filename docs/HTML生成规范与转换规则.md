...
**参考实现**（已验证通过）：请直接查阅 `output/html/ch1_2_5_reference_airport_tab.html` 中的 `<script>` 图片放大区块。
...
**核心原则**：每生成一个新的章节 HTML 后，**必须回溯更新所有已生成的历史 HTML 文件**中的侧边栏导航树，确保导航链接的完整性和一致性。

| 场景 | 必须执行的操作 | 错误示例 | 正确示例 |
|------|--------------|---------|---------|
| 新增 ch1_2_4 | 回溯修改 ch1_2_1 ~ ch1_2_3 的侧边栏，将 1.2.4 的 `href="#"` 改为 `href="ch1_2_4_xxx.html"` | 旧页面中 1.2.4 仍为 `#`，点击无反应 | 旧页面中 1.2.4 指向真实文件 |
| 新增 ch1_2_5 | 回溯修改 ch1_2_1 ~ ch1_2_4 的侧边栏，将 1.2.5 的 `href="#"` 改为 `href="ch1_2_5_xxx.html"` | 旧页面中 1.2.5 仍为 `#`，点击无反应 | 旧页面中 1.2.5 指向真实文件 |
...
- `output/html/ch1_2_5_reference_airport_tab.html`（含最新的图片放大交互与 subgraph 功能层级图实现）
...
- [ ] 页脚 prev/next 链接正确
- [ ] **导航链接回溯更新**：所有已生成的历史 HTML 文件中，新增章节的 `href` 已从 `#` 占位符更新为真实文件路径
