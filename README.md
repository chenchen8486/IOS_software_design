...
4. **`docs/5_IOS工作区架构抽象逻辑分析.html`**（⭐ 架构设计必读）
   - 理解 IOS 六大工作区（Control Board / Map / Lesson Plan Profile / Instruments Repeaters / Maintenance / Advanced）背后的关注点分离、领域建模与认知心理学逻辑。
   - 在进行第 2 章软件架构设计、接口拆分、状态管理设计前，必须优先阅读，确保新增模块与现有架构一致。

5. **`output/html/ch1_8_reference_flows.html`**（⭐ 流程与数据流必读）
   - 理解 6 个参考流程（RF-01 ~ RF-06）的端到端路径、跨工作区数据流、状态一致性约束与边界降级行为。
   - 在实现课程执行、冻结/回放、故障注入等跨模块功能前，必须对照此文档确认接口契约。
...
| 1.8 | 参考流程（Reference Flows） | 无直接对应（基于 1.1~1.7 整合创作） | ✅ V1.1 已完成（架构化重构：数据流、认知模式、异常索引、状态一致性、紧急决策树） |
...
| `docs/5_IOS工作区架构抽象逻辑分析.html` | 基于原始手册的六大工作区架构推演与领域建模分析 | ✅ 已创建 |
