...
5. **`output/html/ch1_8_reference_flows.html`**（⭐ 流程与数据流必读）
   - **1.8.1 ~ 1.8.6**：6 个参考流程（RF-01~RF-06）的端到端路径、跨工作区数据流、状态一致性约束、边界降级行为与紧急决策树。
   - **进入第 2 章前必须通读**：ch1_1 提供「静态架构」，ch1_8 提供「动态流程」，两者共同构成第 2 章的全部输入基础。

6. **`docs/4_第2章设计草案.md`**
   - 在开始第 2 章（软件架构逆向分析）前阅读，了解已确定的架构方向、模块边界与待决策点。

7. **`reference/A320.md`**
   - 定位到当前待处理的章节，提取已整理的功能描述作为内容锚点。

8. **`reference/CAE IOS使用手册.md`（或 `_zh.md`）**
...
| 1.8 | 参考流程（Reference Flows） | 无直接对应（基于 1.1~1.7 整合创作） | ✅ V1.6 已完成（RF-01~RF-06 标准流程 + 课前准备检查清单 + 异常处理参考流程 + 状态一致性设计 + 跨工作区影响矩阵）|
| 1.9 | 索引（Index） | 无直接对应（基于 1.1~1.8 全部页面整合创作） | ✅ 已完成（12 大类别术语索引 + 30 项缩略语速查 + 全页面关联链接）|

### 已完成设计文档

| 文档 | 说明 | 状态 |
|------|------|------|
| `docs/4_第2章设计草案.md` | 第 2 章软件架构逆向分析的设计草案 | ✅ 已创建 |
...
| 2026-06-11 | V0.9 | 优化：1.8 标题移除英文副标（自创章节无原始对应）；开发：新建 1.9 索引页 `ch1_9_index.html`，覆盖 12 大类别（工作区/交互/课程/飞机/机场/环境/故障/通信/状态/地图/架构/缩略语）共 160+ 术语条目，每个术语附中文定义、英文原文与关联 HTML 页面链接；同步更新 README 进度与变更记录；运行 build.py 重新生成 17 个 HTML 产物 |
